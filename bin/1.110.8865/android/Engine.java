...
public final class Engine
{
    ...
    @Keep
    public native int engageInitialize(String enginePolicyConfiguration,
                                       String userIdentity,
    ...
    @Keep
    public native int engageBeginGroupTx(String id,
                                         int txPriority,
                                         int txFlags);

    @Keep
    public native int engageBeginGroupTxAdvanced(String id,
                                                 String jsonParams);
    ...
    @Keep
    public native int engageUpdatePresenceDescriptor(String id,
                                                     String jsonDescriptor,
                                                     int forceBeacon);

    @Keep
    public native int engageSendGroupBlob(String id,
                                          byte[] blob,
                                          int size,
                                          String jsonParams);

    @Keep
    public native int engageSendGroupRtp(String id,
                                         byte[] payload,
                                         int size,
                                         String jsonParams);

    @Keep
    public native int engageSendGroupRaw(String id,
                                         byte[] raw,
                                         int size,
                                         String jsonParams);

    @Keep
    public native int engageRegisterGroupRtpHandler(String id,
                                                    int payloadId);

    @Keep
    public native int engageUnregisterGroupRtpHandler(String id,
                                                      int payloadId);

    @Keep
    public native int engageEncrypt(byte[] src,
                                    int size,
                                    byte[] dst,
    ...
    private void onLicenseExpiring(final String secondsLeft)
    {
        _handler.post(new Runnable()
        {
            ...
            public void run()
            {
                long longSecondsLeft;

                try
                {
                    longSecondsLeft = Long.parseLong(secondsLeft);
                }
                catch(Exception e)
                ...
                {
                    listener.onLicenseExpiring(longSecondsLeft);
                }
            }
        });
    }
